...
//JDT -- Had to add this function
int SlowSoftI2CMaster::i2c_read_multi_byte(uint8_t * buffer, int size)
{
    int receivedBytes = 0;
    if(buffer)
    {
        ...
        {
            if(receivedBytes != size - 1)
            {
                buffer[receivedBytes] = i2c_read(false);
            }
            else
            {
                buffer[receivedBytes] = i2c_read(true);
            }
            receivedBytes++;
        }
    }
...
void SlowSoftI2CMaster::i2c_write_multi_byte(uint8_t * buffer, int size)
{
    int sentBytes = 0;
    if(buffer)
    {
